...
def push(b):
    ...
    if win('X'):
        label['text'] = 'Победили крестики'
    elif win('0'):
        label['text'] = 'Победили нолики'
    else:
        if (len(game_left)) > 1:
            game_left.remove(t)
        else:
...
window = Tk()
label = Label(width=20,text='Крестики против ноликов',font=('Arial', 13, 'bold'))
buttons = [Button(width = 5, height= 2, font=('Arial', 25, 'bold'),command = lambda x=i: push(x)) for i in range(9)]
